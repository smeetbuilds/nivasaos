...
requireText("lib/actions/portal-accounts.js", "passwordInput(formData", "Portal actions do not use shared password validation");
requireText("lib/actions/auth.js", "legacyLocked", "Staff login ignores legacy account locks");
...
for (const file of ["Caddyfile", "next.config.mjs"]) {
  requireText(file, "default-src 'self'", `${file} CSP does not constrain default content sources`);
  requireText(file, "script-src 'self'", `${file} CSP does not constrain executable sources`);
}
...
console.log("Behavioral document authorization, secure invite lifecycle, exact money parsing, historical money preflight, trusted-proxy throttling, strict workflow dates, localization, CSP, Compose loading, and repository cleanup are verified.");
