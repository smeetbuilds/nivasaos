...
export function loadInvoiceWorkspace(user, query) {
  ...
  if (propertyId) { filters.push("p.id=$filterPropertyId"); params.filterPropertyId = propertyId; }
  ...
  const lateFees = canManageBilling ? lateFeeSummary(user) : { count: 0, byCurrency: [] };
  return {
