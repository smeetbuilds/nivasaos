import { createInvoiceAction, createLateFeeRunAction, createRentRunAction, voidInvoiceAction } from "@/app/actions";
import { money, today } from "@/lib/format";
import ModalForm from "@/components/ModalForm";
import Icon from "@/components/Icon";

export default function InvoiceForms({ workspace, lateFeeDetail }) {
  const { properties, leases, tenants, currentPeriod, rentRunStatus, lateFees, canManageBilling, rows } = workspace;
  return <>
    <form action={createInvoiceAction}><ModalForm id="invoice-modal" title="Create an invoice" description="Linking a lease and tenant is recommended for collection, reporting, and reminders." submitLabel="Issue invoice" pendingLabel="Issuing…"><div className="modal-body">
    ...
    {canManageBilling && <form action={createRentRunAction}><ModalForm id="rent-run-modal" title="Run monthly rent billing" description="NivasaOS creates only missing invoices. Running the same property and month again is safe." submitLabel="Generate rent invoices" pendingLabel="Generating…"><div className="modal-body">
    ...
    {canManageBilling && <form action={createLateFeeRunAction}><ModalForm id="late-fee-run-modal" title="Apply eligible late fees" description="Rules are read from each property’s billing policy and recalculated when you submit." submitLabel="Generate late fees" pendingLabel="Generating…"><div className="modal-body">
      ...
      <label><span>Property scope</span><select name="propertyId"><option value="">All accessible properties</option>{properties.map((property) => <option key={property.id} value={property.id}>{property.name}</option>)}</select></label>
    ...
    {canManageBilling && rows.filter((row) => row.status !== "void" && Number(row.amount_paid) === 0).map((row) => <form action={voidInvoiceAction} key={`void-${row.id}`}><ModalForm id={`void-invoice-${row.id}`} title={`Void ${row.number}?`} description="Voiding preserves the audit history but removes this invoice from active receivables." submitLabel="Void invoice" pendingLabel="Voiding…"><div className="modal-body"><input type="hidden" name="invoiceId" value={row.id}/><div className="summary-box"><span>{row.charge_type.replace("_", " ")} invoice</span><strong>{money(row.amount, row.currency)} · {row.description}</strong><small>{row.tenant_name || "Unassigned"} · {row.property_name}</small></div><div className="policy-warning danger-warning">This action is allowed only because no payment is recorded. It cannot be reversed from the interface.</div></div></ModalForm></form>)}
  </>;
}
