  ...
  return <>
    <section className="panel portal-review-panel portal-admin-section" aria-labelledby="portal-payment-review-title">
      ...
      {submissions.length ? <div className="table-wrap"><table className="portal-admin-table" data-mobile-cards="portal-submissions" aria-label="Tenant payment submissions"><thead><tr><th>Submitted</th><th>Tenant / invoice</th><th>Amount</th><th>Payment details</th><th>Status</th><th>Review</th></tr></thead><tbody>{submissions.map((item) => <tr key={item.id}>
        ...
        <td data-label="Review">{item.status === "pending" ? <div className="table-actions portal-review-actions"><form action={reviewPaymentSubmissionAction} className="portal-approval-form"><OpenModalButton target={`approve-submission-${item.id}`} className="button primary small">Approve</OpenModalButton><ModalForm id={`approve-submission-${item.id}`} title={`Approve ${item.tenant_name}'s payment?`} description={`${item.invoice_number || "Unallocated submission"} · ${item.property_name}`} submitLabel="Approve and record" pendingLabel="Approving…"><div className="modal-body"><input type="hidden" name="submissionId" value={item.id}/><input type="hidden" name="decision" value="approved"/><div className="summary-box"><span>Payment submission</span><strong>{money(item.amount, item.currency)} · {item.method.replaceAll("_", " ")}</strong><small>{item.external_reference || "No external reference"}</small></div><div className="portal-approval-note">Approval creates an official payment record and updates the linked invoice balance after live balance validation.</div></div></ModalForm></form><OpenModalButton target={`reject-submission-${item.id}`} className="button secondary small">Reject</OpenModalButton></div> : item.payment_id ? <Link className="text-link" href={`/api/proofs/${item.payment_id}`} target="_blank" rel="noreferrer">Approved proof</Link> : <span className="muted">Reviewed</span>}</td>
