...
export default async function LeasesPage({ searchParams }) {
  ...
  const unitRows = all(
    `SELECT u.id,u.name,u.property_id,u.status,u.monthly_rate,u.deposit,u.capacity,p.name property_name,p.module_id,
     ...
     WHERE ${scope.clause} AND p.status='active' AND u.status NOT IN ('maintenance','inactive') ORDER BY p.name,u.name`,
    scope.params
  );
  ...
  const tenants = all(
    `SELECT t.id,t.full_name,t.property_id,p.name property_name,p.module_id
     FROM tenants t JOIN properties p ON p.id=t.property_id
     WHERE ${scope.clause} AND p.status='active' AND t.status='active' ORDER BY p.name,t.full_name`,
    scope.params
  );
  ...
  return <>
    ...
    <PageHeader eyebrow="Occupancy lifecycle" title="Agreements & stays" description="Residential units remain exclusive. Shared-accommodation rooms can hold concurrent agreements only while validated bed or space inventory remains available." actions={canEdit && <OpenModalButton target="lease-modal">Create agreement</OpenModalButton>}/>
    {leases.length ? <div className="panel"><div className="table-wrap"><table><thead><tr><th>Agreement</th><th>People</th><th>Property / inventory</th><th>Term</th><th>Rent</th><th>Module details</th><th>Status</th><th></th></tr></thead><tbody>{leases.map((lease) => {const module=moduleById(lease.module_id);return <tr key={lease.id}><td><strong>{lease.reference}</strong><small>{module.terminology.agreement} · billing day {lease.billing_day}</small></td><td>{lease.tenant_names || "No linked person"}</td><td><ModuleBadge moduleId={module.id} compact/><strong>{lease.property_name}</strong><small>{lease.unit_name}{lease.allocated_spaces ? ` · ${lease.allocated_spaces}` : ""}</small></td><td>{dateLabel(lease.start_date)}<small>{lease.end_date ? `to ${dateLabel(lease.end_date)}` : "Open ended"}</small></td><td>{money(lease.monthly_rent, lease.currency)}<small>Deposit {money(lease.deposit, lease.currency)}</small></td><td>{lease.active_services} active service{Number(lease.active_services)===1?"":"s"}<small>{module.capabilities.includes("commercialProfiles") ? "Commercial profile required" : module.capabilities.includes("spaceInventory") ? "Space-level occupancy" : "Unit-level occupancy"}</small></td><td><Badge tone={lease.status}>{lease.status}</Badge></td><td>{lease.status === "active" && canEdit && <form action={endLeaseAction}><input type="hidden" name="leaseId" value={lease.id}/><button className="text-button danger-text">Move out</button></form>}</td></tr>})}</tbody></table></div></div> : <Empty icon="lease" title="No agreements created" text="Create a residential lease, shared-accommodation stay, student housing agreement, staff occupancy, or commercial lease."/>}

    {canEdit && <form action={createLeaseAction}><ModalForm id="lease-modal" title="Create an agreement" description="Property, inventory, and people are revalidated server-side. Compatible shared modules allocate one available space per selected resident." submitLabel="Create agreement"><div className="modal-body"><label><span>Property</span><select name="propertyId" required>{properties.map((property) => <option key={property.id} value={property.id}>{moduleById(property.module_id).shortLabel} · {property.name}</option>)}</select><small>The selected inventory and people must belong to this property.</small></label><label><span>Rentable inventory</span><select name="unitId" required>{units.map((unit) => {const module=moduleById(unit.module_id);const shared=supportsCapability(module.id,"spaceInventory");return <option key={unit.id} value={unit.id}>{unit.property_name} · {unit.name} · {shared ? `${unit.available_spaces} of ${unit.configured_spaces} spaces free` : "unit available"}</option>})}</select><small>Shared rooms appear while at least one configured space remains available.</small></label><label><span>People</span><select name="tenantIds" multiple required size="6">{tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.property_name} · {moduleById(tenant.module_id).terminology.occupant} · {tenant.full_name}</option>)}</select><small>Select no more people than available spaces for a shared room. Use Ctrl/Cmd for multiple selection.</small></label><div className="field-grid two"><label><span>Start date</span><input type="date" name="startDate" defaultValue={today()} required/></label><label><span>End date (optional)</span><input type="date" name="endDate"/></label></div><div className="field-grid three"><label><span>Monthly rent</span><input type="number" min="0" step="0.01" name="monthlyRent" required/></label><label><span>Deposit</span><input type="number" min="0" step="0.01" name="deposit"/></label><label><span>Billing day</span><input type="number" min="1" max="28" name="billingDay" defaultValue="1"/></label></div><label><span>Status</span><select name="status"><option value="active">Active</option><option value="draft">Draft</option></select><small>Active shared agreements allocate spaces immediately. Drafts do not reserve inventory.</small></label><label><span>Notes</span><textarea name="notes" rows="3"/></label></div></ModalForm></form>}
  </>;
}
