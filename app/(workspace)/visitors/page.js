...
export default async function VisitorsPage({ searchParams }) {
  ...
  return <>
    ...
    {entries.length ? <section className="panel"><div className="panel-head"><div><span className="eyebrow">Live access desk</span><h2>Visitor movements</h2></div><OpenModalButton target="visitor-create" className="button secondary">New entry</OpenModalButton></div><div className="table-wrap"><table><thead><tr><th>Visitor</th><th>Resident / home</th><th>Expected</th><th>Movement</th><th>Purpose</th><th>Status</th><th>Actions</th></tr></thead><tbody>{entries.map((entry) => <tr key={entry.id}><td><strong>{entry.visitor_name}</strong><small>{entry.visitor_phone || "No phone"}{entry.relationship ? ` · ${entry.relationship}` : ""}</small></td><td><ModuleBadge moduleId={entry.module_id} compact/><strong>{entry.tenant_name}</strong><small>{entry.property_name}{entry.unit_name ? ` · ${entry.unit_name}` : ""}{entry.lease_reference ? ` · ${entry.lease_reference}` : ""}</small></td><td>{dateTimeLabel(entry.expected_at)}<small>{entry.expected_checkout ? `Expected out ${dateTimeLabel(entry.expected_checkout)}` : "No expected checkout"}</small></td><td>{entry.checked_in_at ? `In ${dateTimeLabel(entry.checked_in_at)}` : "Not checked in"}<small>{entry.checked_out_at ? `Out ${dateTimeLabel(entry.checked_out_at)}` : entry.created_by_tenant ? "Resident pre-registration" : `Staff: ${entry.staff_creator || "Former user"}`}</small></td><td>{entry.purpose}<small>{entry.notes || entry.id_reference || "No additional note"}</small></td><td><Badge tone={entry.status === "checked_in" ? "overdue" : entry.status === "checked_out" ? "paid" : entry.status}>{entry.status.replaceAll("_", " ")}</Badge></td><td><div className="table-actions">{entry.status === "expected" && <form action={updateVisitorStatusAction}><input type="hidden" name="visitorId" value={entry.id}/><input type="hidden" name="visitorAction" value="check_in"/><button className="text-button">Check in</button></form>}{entry.status === "checked_in" && <form action={updateVisitorStatusAction}><input type="hidden" name="visitorId" value={entry.id}/><input type="hidden" name="visitorAction" value="check_out"/><button className="text-button">Check out</button></form>}{entry.status === "expected" && <form action={updateVisitorStatusAction}><input type="hidden" name="visitorId" value={entry.id}/><input type="hidden" name="visitorAction" value="cancel"/><button className="text-button danger-text">Cancel</button></form>}</div></td></tr>)}</tbody></table></div></section> : <Empty icon="visitors" title="No visitor activity" text={properties.length ? "Residents can pre-register through their portal, or staff can create the first visitor entry here." : "Enable a compatible accommodation module and create a property first."}/>} 
