  ...
  return <>
    ...
    {leases.map((lease) => {
      ...
      return <form action={saveCommercialProfileAction} key={`profile-${lease.id}`}><ModalForm id={`commercial-profile-${lease.id}`} title={`${lease.profile_id ? "Edit" : "Create"} business profile`} description={`${lease.property_name} · ${lease.unit_name} · ${lease.reference}`} submitLabel="Save commercial profile" pendingLabel="Saving…"><div className="modal-body">
        ...
        <div className="field-grid two"><label><span>Registration number</span><input name="registrationNumber" defaultValue={lease.registration_number || ""} maxLength="120"/></label><label><span>Tax / GST / VAT number</span><input name="taxNumber" defaultValue={lease.tax_number || ""} maxLength="120"/></label></div>
        <label><span>Business activity</span><textarea name="businessActivity" rows="3" maxLength="1200" defaultValue={lease.business_activity || ""}/></label>
