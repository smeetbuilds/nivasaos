...
import { permissionsForUser, portfolioPermissionsForUser } from "@/lib/permissions";
...
export default async function TeamPage({ searchParams }) {
  ...
  return <>
    ...
    <div className="panel"><div className="table-wrap"><table><thead><tr><th>User</th><th>Role</th><th>Property scope</th><th>Effective permissions</th><th>Created</th><th>Status</th><th>Actions</th></tr></thead><tbody>{members.map((member) => { const effective = member.role === "owner" ? PERMISSIONS : portfolioPermissionsForUser(member); return <tr key={member.id}><td><div className="person-cell"><span className="avatar">{member.name[0]}</span><span><strong>{member.name}</strong><small>{member.email}</small></span></div></td><td><Badge tone={member.role}>{member.role}</Badge></td><td>{member.role === "owner" ? "All properties" : member.property_names || "No properties assigned"}</td><td><strong>{effective.length}</strong><small>of {PERMISSIONS.length} capabilities across assigned properties</small></td><td>{dateLabel(member.created_at.slice(0, 10))}</td><td><Badge tone={member.status}>{member.status}</Badge></td><td>{member.role !== "owner" && <div className="row-actions"><OpenModalButton target={`team-edit-${member.id}`} icon="edit" className="text-button">Account</OpenModalButton><OpenModalButton target={`permissions-${member.id}-global`} icon="audit" className="text-button">Permissions</OpenModalButton><form action={toggleUserAction}><input type="hidden" name="userId" value={member.id}/><button className={`text-button ${member.status === "active" ? "danger" : ""}`}>{member.status === "active" ? "Disable" : "Enable"}</button></form></div>}</td></tr>; })}</tbody></table></div></div>
