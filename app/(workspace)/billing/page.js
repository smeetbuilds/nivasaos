...
import { requireRole, propertyScopeSql } from "@/lib/auth";
...
function policyDescription(policy) {
  ...
  const value = policy.late_fee_type === "percent"
    ? `${Number(policy.late_fee_value)}% of outstanding rent`
    : money(policy.late_fee_value, policy.currency);
...
export default async function BillingPage({ searchParams }) {
  ...
  const feeTotal = summary.byCurrency.length === 0
    ? money(0)
    : summary.byCurrency.length === 1
      ? money(summary.byCurrency[0].amount, summary.byCurrency[0].currency)
      : `${summary.byCurrency.length} currencies`;
  const feeDetail = summary.byCurrency.length
    ? summary.byCurrency.map((row) => money(row.amount, row.currency)).join(" · ")
    : "No fees currently eligible";
  ...
  return <>
    ...
    <section className="policy-grid">
      {policies.map((policy) => <article className="panel policy-card" key={policy.id}>
        <div className="panel-head"><div><span className="eyebrow">{policy.currency} property</span><h2>{policy.name}</h2></div><Badge tone={policy.late_fee_type === "none" ? "inactive" : "active"}>{policy.late_fee_type === "none" ? "Disabled" : "Active"}</Badge></div>
