...
export default async function MaintenancePage({ searchParams }) {
  ...
  return <>
    ...
    <PageHeader eyebrow="Work orders" title="Maintenance" description="Track reported issues, communicate with residents, and preserve an operational timeline through resolution." actions={<OpenModalButton target="maintenance-modal" icon="plus">Report issue</OpenModalButton>}/>
    {tickets.length ? <div className="kanban">{columns.map(([status,label]) => <section className="kanban-column" key={status}><div className="kanban-head"><h2>{label}</h2><span>{tickets.filter((ticket)=>ticket.status===status).length}</span></div><div className="kanban-list">{tickets.filter((ticket)=>ticket.status===status).map((ticket)=><article className="ticket-card" key={ticket.id}><div className="ticket-top"><Badge tone={ticket.priority}>{ticket.priority}</Badge><small>{dateTimeLabel(ticket.reported_at)}</small></div><h3>{ticket.title}</h3><p>{ticket.description}</p><div className="ticket-meta"><span>{ticket.property_name}{ticket.unit_name?` · ${ticket.unit_name}`:""}</span><span>{ticket.tenant_name||"No tenant linked"}</span><span>{ticket.assigned_name?`Assigned to ${ticket.assigned_name}`:"Unassigned"}</span></div><div className="ticket-actions"><OpenModalButton target={`ticket-updates-${ticket.id}`} icon="message" className="text-button">Updates ({ticket.comment_count||0})</OpenModalButton>{status!=="reported"&&<form action={updateMaintenanceAction}><input type="hidden" name="ticketId" value={ticket.id}/><input type="hidden" name="status" value={status==="resolved"?"in_progress":"reported"}/><ActionButton className="text-button" pendingLabel="Moving…">Move back</ActionButton></form>}{status!=="resolved"&&<form action={updateMaintenanceAction}><input type="hidden" name="ticketId" value={ticket.id}/><input type="hidden" name="status" value={status==="reported"?"in_progress":"resolved"}/><ActionButton className="text-button" pendingLabel={status==="reported"?"Starting…":"Resolving…"}>{status==="reported"?"Start work":"Resolve"}</ActionButton></form>}</div></article>)}</div></section>)}</div> : <Empty icon="maintenance" title="No maintenance tickets" text="Report an issue and track it through the three-stage operational flow."/>}
