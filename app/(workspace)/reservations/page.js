...
const emptyColumnCopy = {
  checked_in: "No guests are currently checked in.",
  reserved: "No confirmed reservations are waiting for arrival.",
  checked_out: "No completed departures in this view.",
  cancelled: "No cancelled reservations in this view.",
  no_show: "No no-show records in this view."
};
...
export default async function ReservationsPage({ searchParams }) {
  ...
  const boardColumns = statuses.map((id) => ({ id, label: statusLabels[id], count: filteredReservations.filter((item) => item.status === id).length }));
  ...
  return <>
    ...
    {reservations.length > 0 && <form className="panel operations-toolbar" method="get" aria-label="Filter reservations">
      <div className="operations-toolbar-copy"><span className="eyebrow">Front-desk register</span><strong>Reservation board</strong><small aria-live="polite">{filteredReservations.length} of {reservations.length} reservations shown</small></div>
      <div className="operations-filter-grid reservation-filter-grid">
        <label className="operations-search-field"><span>Search reservations</span><input type="search" name="q" defaultValue={query?.q || ""} placeholder="Guest, reference, room, bed, or source"/></label>
        ...
        <label><span>Status</span><select name="status" defaultValue={filters.status}><option value="">All statuses</option>{statuses.map((status) => <option value={status} key={status}>{statusLabels[status]}</option>)}</select></label>
        ...
        <div className="operations-filter-actions"><button className="button secondary" type="submit">Apply filters</button><Link href="/reservations" className="text-link">Reset</Link></div>
      </div>
    </form>}

    {filteredReservations.length ? <OperationsBoard id="reservation-board" label="Reservation status board" columns={boardColumns} className="reservation-board enterprise-reservation-board">
      {statuses.map((status) => {
        const statusItems = filteredReservations.filter((item) => item.status === status);
        return <section id={`reservation-board-${status}`} data-board-column={status} className={`reservation-column reservation-column-${status}`} key={status} aria-labelledby={`reservation-${status}-heading`}>
          <div className="reservation-column-head"><div><span className="eyebrow">Stay status</span><strong id={`reservation-${status}-heading`}>{statusLabels[status]}</strong></div><span>{statusItems.length}</span></div>
          <div className="reservation-list">{statusItems.length ? statusItems.map((item) => {
            ...
            const attention = item.status === "reserved" && item.arrival_date === today() ? "Arrival today" : item.status === "checked_in" && item.departure_date === today() ? "Departure today" : null;
            const titleId = `reservation-guest-${item.id}`;
            const historical = ["checked_out", "cancelled", "no_show"].includes(item.status);
            return <article className="reservation-card enterprise-reservation-card" aria-labelledby={titleId} key={item.id}>
              <div className="reservation-card-top"><Badge tone={item.status}>{statusLabels[item.status]}</Badge><small className="reservation-reference">{item.reference}</small></div>
              {attention && <span className="reservation-attention">{attention}</span>}
              <h2 id={titleId}>{item.guest_name}</h2><p>{item.property_name} · {item.unit_name || "Unassigned room"}{item.space_code ? ` · ${item.space_code}` : ""}</p>
              ...
              <div className="reservation-actions">
                {item.status === "reserved" && <><form action={updateHostelReservationStatusAction}><input type="hidden" name="reservationId" value={item.id}/><input type="hidden" name="status" value="checked_in"/><ActionButton className="button primary small" pendingLabel="Checking in…">Check in</ActionButton></form><TransitionConfirmation item={item} status="no_show" label="Mark no-show" title="Mark this guest as a no-show?" triggerClassName="button secondary small" consequence="This releases the reservation from the active arrival queue and records the no-show transition in the audit history."/><TransitionConfirmation item={item} status="cancelled" label="Cancel reservation" title="Cancel this reservation?" consequence="This removes the reservation from active inventory. The original reservation record remains in history."/></>}
                {item.status === "checked_in" && <TransitionConfirmation item={item} status="checked_out" label="Check out" title="Check out this guest?" triggerClassName="button primary small" consequence="Checkout ends the active stay and automatically creates the associated housekeeping turnover task."/>}
                {historical && <span className="operations-history-note">Historical reservation record</span>}
              </div>
            </article>;
          }) : <div className="kanban-empty board-column-empty">{hasActiveFilters ? "No matching reservations in this status." : emptyColumnCopy[status]}</div>}</div>
        </section>;
      })}
    </OperationsBoard> : reservations.length ? <Empty icon="hostel" title="No reservations match these filters" text="Adjust the guest search, hostel, status, or working-window filters to view more stays."/> : <Empty icon="hostel" title="No hostel reservations" text={emptyText}/>} 

    {properties.length > 0 && <form action={createHostelReservationAction}><ModalForm id="reservation-create" title="Create hostel reservation" description="The selected bed is checked for every overlapping active reservation before insertion." submitLabel="Reserve stay" pendingLabel="Reserving…"><div className="modal-body"><label><span>Hostel</span><select name="propertyId" required>{properties.map((property) => <option value={property.id} key={property.id}>{property.name}</option>)}</select></label><div className="field-grid two"><label><span>Room / dorm</span><select name="unitId"><option value="">Assign later</option>{units.map((unit) => <option value={unit.id} key={unit.id}>{unit.property_name} · {unit.name}</option>)}</select></label><label><span>Bed</span><select name="spaceId"><option value="">Assign later</option>{spaces.map((space) => <option value={space.id} key={space.id}>{space.property_name} · {space.unit_name} · {space.code}</option>)}</select></label></div><div className="field-grid two"><label><span>Guest name</span><input name="guestName" autoComplete="name" required/></label><label><span>Adults</span><input type="number" min="1" name="adults" defaultValue="1"/></label></div><div className="field-grid two"><label><span>Email</span><input type="email" name="guestEmail" autoComplete="email"/></label><label><span>Phone</span><input name="guestPhone" inputMode="tel" autoComplete="tel"/></label></div><div className="field-grid two"><label><span>Arrival</span><input type="date" name="arrivalDate" defaultValue={today()} required/></label><label><span>Departure</span><input type="date" name="departureDate" required/></label></div><div className="field-grid three"><label><span>Nightly rate</span><input type="number" min="0" step="0.01" name="nightlyRate" defaultValue="0"/></label><label><span>Tax amount</span><input type="number" min="0" step="0.01" name="taxAmount" defaultValue="0"/></label><label><span>Source</span><select name="source"><option value="direct">Direct</option><option value="walk_in">Walk-in</option><option value="phone">Phone</option><option value="group">Group</option><option value="channel">External channel</option></select></label></div><label><span>Identity / passport reference</span><input name="identityReference"/></label><label><span>Notes</span><textarea name="notes" rows="3"/></label></div></ModalForm></form>}
  </>;
}
