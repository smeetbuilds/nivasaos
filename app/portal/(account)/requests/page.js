...
export default async function PortalRequestsPage({ searchParams }) {
  ...
  return <>
    ...
    <section className="portal-metric-grid"><article><span>Awaiting review</span><strong>{pending}</strong><small>Submitted to the property team</small></article><article><span>Approved in progress</span><strong>{approved}</strong><small>Waiting for completion</small></article><article><span>Completed</span><strong>{requests.filter((item)=>item.status==="completed").length}</strong><small>Retained in your history</small></article><article><span>Available request types</span><strong>{contract.portalActions.length}</strong><small>Configured for {module.shortLabel}</small></article></section>

    {profile && <section className={`portal-card portal-domain-profile module-${module.id}`}><div className="portal-card-head"><div><span className="eyebrow">Your module profile</span><h2>{contract.profileTitle}</h2></div><Icon name={module.icon} size={22}/></div><div className="portal-detail-grid portal-domain-grid">{profile.external_id&&<span><small>Reference ID</small><strong>{profile.external_id}</strong></span>}{profile.organisation&&<span><small>Organisation</small><strong>{profile.organisation}</strong></span>}{profile.department&&<span><small>Department</small><strong>{profile.department}</strong></span>}{profile.programme&&<span><small>Programme</small><strong>{profile.programme}</strong></span>}{profile.level_or_designation&&<span><small>Level / designation</small><strong>{profile.level_or_designation}</strong></span>}{profile.guardian_name&&<span><small>Guardian</small><strong>{profile.guardian_name}</strong></span>}{profile.sponsor_name&&<span><small>Sponsor</small><strong>{profile.sponsor_name}</strong></span>}{profile.eligibility_end_date&&<span><small>Eligibility ends</small><strong>{profile.eligibility_end_date}</strong></span>}</div></section>}

    <section className="portal-card"><div className="portal-card-head"><div><span className="eyebrow">Workflow history</span><h2>Your requests</h2></div></div>{requests.length ? <div className="portal-request-list">{requests.map((request) => <article key={request.id}><span className="portal-request-icon"><Icon name="portal" size={18}/></span><span><strong>{request.title}</strong><small>{requestLabel(request.request_type)} · submitted {dateTimeLabel(request.created_at)}</small><p>{request.details || "No additional detail"}</p>{request.resolution_note&&<em>{request.resolution_note}</em>}</span><span className="portal-request-state"><Badge tone={request.status}>{request.status}</Badge>{request.starts_at&&<small>{dateTimeLabel(request.starts_at)}{request.ends_at?` to ${dateTimeLabel(request.ends_at)}`:""}</small>}{request.status==="submitted"&&<form action={cancelTenantModuleRequestAction}><input type="hidden" name="requestId" value={request.id}/><button className="text-button danger-text">Cancel</button></form>}</span></article>)}</div> : <div className="portal-empty-state"><Icon name="portal" size={28}/><strong>No requests yet</strong><p>Use this centre for {contract.portalActions.map(requestLabel).join(", ")}.</p></div>}</section>
