    ...
    <section className="portal-card">
      ...
      {handover.inspections.length ? <div className="portal-inspection-list">{handover.inspections.map((inspection) => {
        ...
        return <article className="portal-inspection-card" key={inspection.id}>
          ...
          <div className="portal-inspection-foot"><span>{inspection.acknowledged_at ? `Acknowledged ${dateTimeLabel(inspection.acknowledged_at)}` : "Awaiting your acknowledgement"}</span>{!inspection.acknowledged_at && ["shared", "acknowledged"].includes(inspection.status) && <OpenModalButton target={`ack-inspection-${inspection.id}`} className="button secondary">Review and acknowledge</OpenModalButton>}</div>
        </article>;
