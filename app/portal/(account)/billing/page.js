...
export default async function PortalBillingPage({ searchParams }) {
  ...
  return <>
    ...
    <section className="portal-metric-grid portal-billing-metrics">
      <article><span>Outstanding balance</span><strong>{outstandingLabel}</strong><small>Across issued and part-paid invoices</small></article>
      ...
      <article><span>Submissions pending</span><strong>{data.submissions.filter((item) => item.status === "pending").length}</strong><small>Reserved from duplicate proof submissions</small></article>
    </section>

    <section className="portal-card">
      <div className="portal-card-head"><div><span className="eyebrow">What you owe</span><h2>Invoices</h2></div>{payable.length > 0 && <OpenModalButton target="portal-payment-modal" className="button secondary" icon="plus">Submit proof</OpenModalButton>}</div>
      {data.invoices.length ? <div className="portal-invoice-list">{data.invoices.map((invoice) => {
        ...
        return <article key={invoice.id} className={`portal-invoice-card${overdue ? " is-overdue" : ""}`}>
          <div><span className="portal-invoice-icon"><Icon name={invoice.charge_type === "rent" ? "home" : invoice.charge_type === "late_fee" ? "billing" : "receipt"} size={20}/></span><span><strong>{invoice.number}</strong><small>{invoice.description} · {invoice.property_name}{invoice.unit_name ? ` · ${invoice.unit_name}` : ""}</small></span></div>
          ...
          <div><span>Due {dateLabel(invoice.due_date)}</span>{pending > 0 && <small>{money(pending, invoice.currency)} proof pending</small>}</div>
    ...
    <section className="portal-card">
      <div className="portal-card-head"><div><span className="eyebrow">Review queue</span><h2>Proof submissions</h2></div></div>
      {data.submissions.length ? <div className="portal-submission-list">{data.submissions.map((item) => <article key={item.id}><span className="portal-submission-status"><Badge tone={item.status}>{item.status}</Badge></span><span><strong>{money(item.amount, item.currency)}</strong><small>{item.invoice_number || "No invoice"} · submitted {dateTimeLabel(item.created_at)}</small></span><span><strong>{item.method.replaceAll("_", " ")}</strong><small>{item.external_reference || "No reference"}</small></span><span className="portal-submission-actions"><a href={`/portal/proofs/submissions/${item.id}`} target="_blank" className="text-link">View proof</a>{item.status === "approved" && item.payment_id && <Link href={`/portal/receipts/${item.payment_id}`} className="text-link">Receipt</Link>}{item.status === "pending" && <form action={cancelTenantPaymentSubmissionAction}><input type="hidden" name="submissionId" value={item.id}/><button className="text-button danger-text">Cancel</button></form>}</span>{item.review_note && <p>{item.review_note}</p>}</article>)}</div> : <div className="portal-empty-state compact"><strong>No proof submissions</strong><p>Use “Submit payment proof” after paying an invoice outside the portal.</p></div>}
    </section>

    <section className="portal-card">
      <div className="portal-card-head"><div><span className="eyebrow">Official ledger</span><h2>Payment receipts</h2></div></div>
    ...
    <form action={submitTenantPaymentAction}><ModalForm id="portal-payment-modal" title="Submit payment proof" description="This does not mark an invoice paid. Staff review the proof and then create the official payment receipt." submitLabel="Submit for review" pendingLabel="Uploading…"><div className="modal-body">
