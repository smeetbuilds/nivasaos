...
export default async function TenantPortalHome({ searchParams }) {
  ...
  const openInvoiceCount = data.outstanding.reduce((sum, row) => sum + Number(row.invoice_count || 0), 0);
  const homeLabel = module.id === "commercial" ? "premises" : "home";
  return <>
    <Flash searchParams={query}/>
    {query?.welcome && <section className={`portal-welcome module-${module.id}`} aria-labelledby="portal-welcome-title"><div><span className="eyebrow">{module.terminology.portal} secured</span><h1 id="portal-welcome-title">Welcome, {tenant.full_name.split(" ")[0]}.</h1><p>Your password is active and the one-time setup link has been consumed.</p></div><Icon name={module.icon} size={34}/></section>}
    ...
    <section className="portal-metric-grid" aria-label="Resident account summary">
      <article className={openInvoiceCount ? "is-risk" : ""}><span>Outstanding</span><strong>{currencySummary(data.outstanding, "balance")}</strong><small>{openInvoiceCount} open invoice{openInvoiceCount === 1 ? "" : "s"}</small></article>
      ...
      <article className={data.pendingSubmissions ? "is-risk" : ""}><span>Payment proofs</span><strong>{data.pendingSubmissions}</strong><small>Awaiting property-team review</small></article>
      <article className={data.openTickets ? "is-risk" : ""}><span>Open maintenance</span><strong>{data.openTickets}</strong><small>Reported or in progress</small></article>
    </section>

    {(activeSpace || activeServices.length || visitorsInside || activeCommercial) && <section className="portal-module-summary-grid" aria-label="Resident services summary">
    ...
    <section className="portal-dashboard-grid" aria-label="Resident account details">
      <article className="portal-card portal-home-card" aria-labelledby="portal-home-title">
        <div className="portal-card-head"><div><span className="eyebrow">Current {homeLabel}</span><h2 id="portal-home-title">{lease ? `${lease.property_name} · ${lease.unit_name}` : `No active ${homeLabel}`}</h2></div>{lease && <Badge tone="active">Active {module.terminology.agreement.toLowerCase()}</Badge>}</div>
      ...
      <article className="portal-card portal-due-card" aria-labelledby="portal-next-action-title">
        <div className="portal-card-head"><div><span className="eyebrow">Next action</span><h2 id="portal-next-action-title">{data.nextInvoice ? "Payment due" : "Account up to date"}</h2></div><Icon name="receipt" size={24}/></div>
      ...
      <article className="portal-card portal-span-2" aria-labelledby="portal-recent-payments-title"><div className="portal-card-head"><div><span className="eyebrow">Receipts</span><h2 id="portal-recent-payments-title">Recent payments</h2></div><Link href="/portal/billing" className="text-link">View all</Link></div>{data.recentPayments.length ? <div className="portal-activity-list">{data.recentPayments.map((payment) => <Link href={`/portal/receipts/${payment.id}`} key={payment.id}><span className="portal-activity-icon"><Icon name="receipt" size={18}/></span><span><strong>{payment.reference}</strong><small>{payment.invoice_number || "Unallocated payment"} · {dateLabel(payment.paid_at)}{payment.payer_name && payment.payer_name !== tenant.full_name ? ` · paid by ${payment.payer_name}` : ""}</small></span><strong>{money(payment.amount, payment.currency)}</strong></Link>)}</div> : <div className="portal-empty-state compact"><strong>No payments recorded yet</strong><p>Approved payments and downloadable receipts will appear here.</p></div>}</article>
    </section>
  </>;
}
