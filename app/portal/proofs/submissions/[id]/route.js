...
export async function GET(_request, { params }) {
  ...
  const submission = get("SELECT proof_path FROM payment_submissions WHERE id=$id AND tenant_id=$tenantId", { id: Number(id), tenantId: tenant.tenant_id });
  if (!submission?.proof_path) return new Response("Not found", { status: 404 });
