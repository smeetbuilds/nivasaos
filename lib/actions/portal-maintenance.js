...
export async function createTenantMaintenanceAction(formData) {
  ...
  transaction(() => {
    const result = run(
      `INSERT INTO maintenance_tickets (property_id,unit_id,tenant_id,title,description,priority,status)
       VALUES ($propertyId,$unitId,$tenantId,$title,$description,$priority,'reported')`,
      { propertyId: lease.property_id, unitId: lease.unit_id, tenantId: tenant.tenant_id, title, description, priority }
    );
...
export async function addTenantMaintenanceCommentAction(formData) {
  ...
  transaction(() => {
    const result = run(
      `INSERT INTO maintenance_comments (ticket_id,actor_tenant_id,message,visibility)
       VALUES ($ticketId,$tenantId,$message,'tenant')`,
      { ticketId, tenantId: tenant.tenant_id, message }
    );
  ...
  transaction(() => {
    const result = run(
      `INSERT INTO maintenance_comments (ticket_id,actor_user_id,message,visibility)
       VALUES ($ticketId,$userId,$message,$visibility)`,
      { ticketId, userId: actor.id, message, visibility }
    );
    run("UPDATE maintenance_tickets SET updated_at=CURRENT_TIMESTAMP WHERE id=$ticketId", { ticketId });
