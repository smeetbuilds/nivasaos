...
export async function createTenantInviteAction(formData) {
  ...
  const handoff = Buffer.from(JSON.stringify({ token, tenantId, createdAt: Date.now() }), "utf8").toString("base64url");
  const store = await cookies();
  store.set(PORTAL_HANDOFF_COOKIE, handoff, {
    httpOnly: true,
    sameSite: "strict",
...
export async function activateTenantPortalAction(formData) {
  ...
  const confirmation = rawPassword(formData, "confirmPassword");
...
export async function tenantLoginAction(formData) {
  ...
  const passwordValid = verifyPasswordOrDummy(password, account?.password_hash);
  const valid = retryAfter === 0 && Boolean(account && account.status === "active" && passwordValid);
  if (!valid) {
