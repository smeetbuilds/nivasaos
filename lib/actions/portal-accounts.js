...
import { canAccessProperty, hashPassword, requireRole, verifyPassword } from "@/lib/auth";
...
export async function createTenantInviteAction(formData) {
  ...
  if (!tenant || !canAccessProperty(actor, tenant.property_id)) throw new Error("Tenant access denied");
  ...
  void accountId;
  refreshPortalViews();
  redirect(`/tenant-portal?success=${encodeURIComponent("Portal link created. Share it securely with the tenant.")}&invite=${encodeURIComponent(token)}&tenant=${tenantId}`);
}
...
export async function tenantLoginAction(formData) {
  ...
  const now = new Date();
  const locked = account?.locked_until && new Date(account.locked_until) > now;
  const valid = account && account.status === "active" && !locked && account.password_hash && verifyPassword(password, account.password_hash);
  if (!valid) {
    if (account && account.status === "active" && !locked) {
      run(
        `UPDATE tenant_accounts SET failed_attempts=failed_attempts+1,
         locked_until=CASE WHEN failed_attempts+1>=5 THEN $lockUntil ELSE NULL END,updated_at=CURRENT_TIMESTAMP
         WHERE id=$accountId`,
        { lockUntil: new Date(Date.now() + 15 * 60000).toISOString(), accountId: account.id }
      );
    }
    redirect("/portal/login?error=Invalid%20email%20or%20password");
  }
