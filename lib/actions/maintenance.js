...
export async function createMaintenanceAction(formData) {
  ...
  assertPermission(actor, "maintenance.manage", propertyId);
  ...
  if (assignedTo && !get(`SELECT 1 FROM users u WHERE u.id=$assignedTo AND u.status='active' AND (u.role='owner' OR EXISTS (SELECT 1 FROM user_properties up WHERE up.user_id=u.id AND up.property_id=$propertyId))`, { assignedTo, propertyId })) throw new Error("Assignee does not have access to this property");
  ...
  transaction(() => {
    const result = run(`INSERT INTO maintenance_tickets (property_id,unit_id,tenant_id,title,description,priority,status,assigned_to) VALUES ($propertyId,$unitId,$tenantId,$title,$description,$priority,'reported',$assignedTo)`, { propertyId, unitId, tenantId, title, description, priority, assignedTo });
    const ticketId = Number(result.lastInsertRowid);
    if (tenantId) run(`INSERT INTO maintenance_comments (ticket_id,actor_user_id,message,visibility) VALUES ($ticketId,$userId,$message,'tenant')`, { ticketId, userId: actor.id, message: "Your property team created this maintenance request and will post updates here." });
...
export async function updateMaintenanceAction(formData) {
  ...
  transaction(() => {
    run("UPDATE maintenance_tickets SET status=$status,resolved_at=CASE WHEN $status='resolved' THEN CURRENT_TIMESTAMP ELSE NULL END,updated_at=CURRENT_TIMESTAMP WHERE id=$ticketId", { status, ticketId });
    if (ticket.tenant_id) {
      ...
      run(`INSERT INTO maintenance_comments (ticket_id,actor_user_id,message,visibility) VALUES ($ticketId,$userId,$message,'tenant')`, { ticketId, userId: actor.id, message: label });
    }
