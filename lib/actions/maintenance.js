...
export async function createMaintenanceAction(formData) {
  ...
  transaction(() => {
    const result = run(
      ...
      { propertyId, unitId, tenantId, title, description: text(formData, "description", true), priority, assignedTo }
    );
    recordAudit({ actor, action: "create", entityType: "maintenance_ticket", entityId: Number(result.lastInsertRowid), propertyId, summary: `Reported maintenance issue: ${title}`, metadata: { priority, unitId, assignedTo } });
  });
