...
export async function submitTenantPaymentAction(formData) {
  ...
  try {
    transaction(() => {
      ...
      const pendingMinor = toMinorUnits(scalar("SELECT COALESCE(SUM(amount),0) FROM payment_submissions WHERE invoice_id=$invoiceId AND status='pending'", { invoiceId }) || 0, "Pending payment amount");
