...
import { fromMinorUnits, MAX_MONEY_MINOR, moneyInput, toMinorUnits } from "@/lib/money";
...
export async function submitTenantPaymentAction(formData) {
  ...
  const amount = moneyInput(formData, "amount", { label: "Payment amount", minMinor: 1 });
  ...
  try {
    transaction(() => {
      ...
      const invoiceMinor = toMinorUnits(currentInvoice.amount, "Invoice amount");
      const paidMinor = toMinorUnits(currentInvoice.amount_paid, "Invoice paid amount");
      const pendingMinor = Number(scalar(
        `SELECT COALESCE(SUM(CAST(ROUND(amount * 100) AS INTEGER)),0)
         FROM payment_submissions WHERE invoice_id=$invoiceId AND status='pending'`,
        { invoiceId }
      ) || 0);
      if (!Number.isSafeInteger(pendingMinor) || pendingMinor < 0 || pendingMinor > MAX_MONEY_MINOR) {
        throw new Error("Pending payment total is outside the supported monetary range");
      }
      if (amount.minor > invoiceMinor - paidMinor - pendingMinor) throw new Error("Amount exceeds the invoice balance remaining after pending submissions");
      const result = run(
      ...
      recordAudit({ tenantActor: tenant, action: "create", entityType: "payment_submission", entityId: Number(result.lastInsertRowid), propertyId: invoice.property_id, summary: `${tenant.full_name} submitted payment proof for ${invoice.number}`, metadata: { amount: amount.value, amountMinor: amount.minor, invoiceId } });
    });
  } catch (error) {
...
export async function cancelTenantPaymentSubmissionAction(formData) {
  ...
  transaction(() => {
    const changed = run("UPDATE payment_submissions SET status='cancelled',updated_at=CURRENT_TIMESTAMP WHERE id=$submissionId AND tenant_id=$tenantId AND status='pending'", { submissionId, tenantId: tenant.tenant_id });
    if (Number(changed.changes) !== 1) throw new Error("Submission status changed before cancellation completed");
...
export async function reviewPaymentSubmissionAction(formData) {
  ...
  transaction(() => {
    ...
    if (decision === "approved") {
      ...
      const paymentMinor = toMinorUnits(current.amount, "Submitted payment amount");
      if (paymentMinor > invoiceMinor - paidMinor) throw new Error("Payment now exceeds the invoice balance");
      ...
      const newPaidMinor = paidMinor + paymentMinor;
      const newPaid = fromMinorUnits(newPaidMinor);
      const newStatus = newPaidMinor === invoiceMinor ? "paid" : "part_paid";
      const invoiceChanged = run(
        ...
        { newPaid, newStatus, invoiceId: current.invoice_id, currentPaid: invoice.amount_paid, currentStatus: invoice.status }
      );
      if (Number(invoiceChanged.changes) !== 1) throw new Error("Invoice balance changed before approval completed");
    }
    const reviewed = run(
      `UPDATE payment_submissions SET status=$decision,review_note=$reviewNote,reviewed_by=$reviewedBy,
       reviewed_at=CURRENT_TIMESTAMP,payment_id=$paymentId,updated_at=CURRENT_TIMESTAMP WHERE id=$submissionId AND status='pending'`,
      { decision, reviewNote, reviewedBy: actor.id, paymentId, submissionId }
    );
    if (Number(reviewed.changes) !== 1) throw new Error("Submission was reviewed by another request");
    recordAudit({ actor, action: decision === "approved" ? "record" : "status", entityType: "payment_submission", entityId: submissionId, propertyId: current.property_id, summary: `${decision === "approved" ? "Approved" : "Rejected"} tenant payment submission`, metadata: { amount: current.amount, amountMinor: toMinorUnits(current.amount), invoiceId: current.invoice_id, paymentId } });
  });
  refreshPortalViews();
