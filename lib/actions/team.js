...
export async function updateTeamMemberAction(formData) {
  ...
  transaction(() => {
    ...
    if (propertyIds.length) {
      run(`UPDATE maintenance_tickets SET assigned_to=NULL,updated_at=CURRENT_TIMESTAMP WHERE assigned_to=? AND status!='resolved' AND property_id NOT IN (${propertyIds.map(() => "?").join(",")})`, [userId, ...propertyIds]);
    } else {
...
export async function updateUserPermissionsAction(formData) {
  ...
  const allowed = new Set(formData.getAll("permissions").map(String).filter((permission) => PERMISSIONS.includes(permission)));
  transaction(() => {
    ...
    for (const permission of PERMISSIONS) {
      run(
        "INSERT INTO permission_grants (user_id,property_id,permission,allowed,granted_by) VALUES ($userId,$propertyId,$permission,$allowed,$actorId)",
        { userId, propertyId, permission, allowed: allowed.has(permission) ? 1 : 0, actorId: actor.id }
      );
    }
