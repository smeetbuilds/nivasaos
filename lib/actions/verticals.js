...
const HOUSEKEEPING_STATUSES = ["open", "in_progress", "blocked", "completed", "cancelled"];
...
function validDate(value, label, required = false) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || Number.isNaN(Date.parse(`${date}T00:00:00Z`))) throw new Error(`${label} must be a valid date`);
  return date;
}
...
export async function cancelTenantModuleRequestAction(formData) {
  ...
  transaction(() => {
    run("UPDATE module_requests SET status='cancelled',updated_at=CURRENT_TIMESTAMP WHERE id=$requestId AND status='submitted'", { requestId });
...
export async function reviewModuleRequestAction(formData) {
  ...
  transaction(() => {
    run(
      `UPDATE module_requests SET status=$status,resolution_note=$note,reviewed_by=$actorId,reviewed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=$requestId`,
      { status, note: limited(formData, "resolutionNote", 2000), actorId: actor.id, requestId }
    );
...
export async function updateHostelReservationStatusAction(formData) {
  ...
  transaction(() => {
    run("UPDATE hostel_reservations SET status=$next,updated_at=CURRENT_TIMESTAMP WHERE id=$reservationId", { next, reservationId });
    if (next === "checked_out") {
      run(
        `INSERT INTO housekeeping_tasks (property_id,unit_id,space_id,reservation_id,task_type,priority,due_at,status,notes,created_by)
         VALUES ($propertyId,$unitId,$spaceId,$reservationId,'turnover','high',CURRENT_TIMESTAMP,'open','Automatic checkout turnover',$actorId)`,
...
export async function updateHousekeepingTaskAction(formData) {
  ...
  const status = choice(formData, "status", HOUSEKEEPING_STATUSES);
  ...
  transaction(() => {
    run(
      `UPDATE housekeeping_tasks SET status=$status,completed_at=CASE WHEN $status='completed' THEN CURRENT_TIMESTAMP ELSE NULL END,
       notes=CASE WHEN $notes='' THEN notes ELSE $notes END,updated_at=CURRENT_TIMESTAMP WHERE id=$taskId`,
      { status, notes: limited(formData, "notes", 2000), taskId }
    );
    recordAudit({ actor, action: "status", entityType: "housekeeping_task", entityId: taskId, propertyId: task.property_id, summary: `Moved housekeeping task to ${status.replaceAll("_", " ")}`, metadata: { from: task.status, to: status } });
  });
  refreshVerticals();
...
export async function bulkServiceBillingAction(formData) {
  ...
  const preview = eligible.map((item) => ({ subscriptionId: item.id, service: item.service_name, agreement: item.lease_reference, amount: Number(item.custom_amount ?? item.default_amount) }));
  if (mode === "preview") {
    ...
    refreshVerticals();
    safeRedirect("/services", `${eligible.length} service invoice(s) ready for ${period}`);
  }
  ...
  transaction(() => {
    ...
    for (const item of eligible) {
      ...
      created.push({ subscriptionId: item.id, invoiceId, invoiceNumber, amount });
    }
    run("UPDATE bulk_jobs SET status='completed',result_json=$result,completed_at=CURRENT_TIMESTAMP WHERE idempotency_key=$key", { result: JSON.stringify(created), key: idempotencyKey });
    recordAudit({ actor, action: "generate", entityType: "bulk_service_billing", propertyId, summary: `Generated ${created.length} service invoices for ${period}`, metadata: { period, count: created.length, idempotencyKey } });
  });
  refreshVerticals();
  safeRedirect("/services", `${eligible.length} service invoice(s) generated`);
}
