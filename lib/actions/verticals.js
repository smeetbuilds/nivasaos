...
export async function bulkServiceBillingAction(formData) {
  ...
  const billable = eligible.filter((item) => toMinorUnits(item.custom_amount ?? item.default_amount, "Service amount") > 0);
  const preview = billable.map((item) => ({
    subscriptionId: item.id,
    service: item.service_name,
    agreement: item.lease_reference,
    amount: normalizedMoney(item.custom_amount ?? item.default_amount, "Service amount")
  }));
  if (mode === "preview") {
  ...
  const created = transaction(() => {
    ...
    for (const item of billable) {
      const amount = normalizedMoney(item.custom_amount ?? item.default_amount, "Service amount");
