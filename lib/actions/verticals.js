...
function validDateTime(value, label, required = false) {
  ...
  if (local) {
    ...
    return `${local[1]}T${local[2]}:${local[3]}${local[4] ? `:${local[4]}` : ""}`;
  }
...
export async function updateHousekeepingTaskAction(formData) {
  ...
  if (!HOUSEKEEPING_TRANSITIONS[task.status]?.includes(status)) throw new Error("Invalid housekeeping transition");
  ...
  transaction(() => {
    ...
    if (Number(changed.changes) !== 1) throw new Error("Housekeeping status changed before this transition completed");
