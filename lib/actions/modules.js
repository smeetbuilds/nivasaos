...
export async function updateWorkspaceModulesAction(formData) {
  ...
  const disabledInUse = all(`SELECT module_id,COUNT(*) property_count FROM properties WHERE module_id NOT IN (${selected.map(() => "?").join(",")}) GROUP BY module_id`, selected);
  if (disabledInUse.length) throw new Error(`Cannot disable modules used by properties: ${disabledInUse.map((row) => `${row.module_id} (${row.property_count})`).join(", ")}`);
  ...
  transaction(() => {
    MODULE_CATALOG.forEach((module, index) => run(`INSERT INTO workspace_modules (module_id,enabled,sort_order,updated_at) VALUES ($moduleId,$enabled,$sortOrder,CURRENT_TIMESTAMP) ON CONFLICT(module_id) DO UPDATE SET enabled=excluded.enabled,sort_order=excluded.sort_order,updated_at=CURRENT_TIMESTAMP`, { moduleId: module.id, enabled: selected.includes(module.id) ? 1 : 0, sortOrder: index * 10 + 10 }));
    run(`INSERT INTO settings (key,value,updated_at) VALUES ('primary_module',$primaryModule,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP`, { primaryModule });
