...
function accessibleSpace(actor, spaceId, permissions = ["inventory.manage"]) {
  const space = get(
  ...
  for (const permission of permissions) assertPermission(actor, permission, space.property_id);
...
export async function createSpaceAction(formData) {
  ...
  assertPermission(actor, "inventory.manage", propertyId);
  ...
  const values = { propertyId, unitId, code: limited(formData, "code", 120, true), spaceType: choice(formData, "spaceType", SPACE_TYPES, "bed"), monthlyRate: Math.max(0, number(formData, "monthlyRate", unit.monthly_rate)), deposit: Math.max(0, number(formData, "deposit", unit.deposit)), genderPolicy: choice(formData, "genderPolicy", GENDER_POLICIES, "any"), status: choice(formData, "status", ["available", "maintenance", "inactive"], "available"), notes: limited(formData, "notes", 1500) };
  transaction(() => {
    const represented = Number(scalar("SELECT COUNT(*) FROM rentable_spaces WHERE unit_id=$unitId AND status!='inactive'", { unitId }) || 0);
    if (values.status !== "inactive" && represented >= Number(unit.capacity)) throw new Error("Unit capacity is already fully represented by spaces");
    const inserted = run(`INSERT INTO rentable_spaces (property_id,unit_id,code,space_type,monthly_rate,deposit,gender_policy,status,notes) VALUES ($propertyId,$unitId,$code,$spaceType,$monthlyRate,$deposit,$genderPolicy,$status,$notes)`, values);
...
export async function updateSpaceAction(formData) {
  ...
  const after = { code: limited(formData, "code", 120, true), space_type: choice(formData, "spaceType", SPACE_TYPES, before.space_type), monthly_rate: Math.max(0, number(formData, "monthlyRate", before.monthly_rate)), deposit: Math.max(0, number(formData, "deposit", before.deposit)), gender_policy: choice(formData, "genderPolicy", GENDER_POLICIES, before.gender_policy), status: requestedStatus, notes: limited(formData, "notes", 1500) };
  ...
  transaction(() => {
    if (before.status === "inactive" && after.status !== "inactive") {
      const represented = Number(scalar("SELECT COUNT(*) FROM rentable_spaces WHERE unit_id=$unitId AND id!=$spaceId AND status!='inactive'", { unitId: before.unit_id, spaceId }) || 0);
      if (represented >= Number(before.capacity)) throw new Error("Reactivating this space would exceed unit capacity");
    }
    run(`UPDATE rentable_spaces SET code=$code,space_type=$space_type,monthly_rate=$monthly_rate,deposit=$deposit,gender_policy=$gender_policy,status=$status,notes=$notes,updated_at=CURRENT_TIMESTAMP WHERE id=$spaceId`, { ...after, spaceId });
...
export async function allocateSpaceAction(formData) {
  ...
  const space = accessibleSpace(actor, spaceId, ["inventory.manage", "agreements.manage"]);
  ...
  transaction(() => {
    ...
    const inserted = run(`INSERT INTO space_allocations (property_id,space_id,lease_id,tenant_id,start_date,status,created_by) VALUES ($propertyId,$spaceId,$leaseId,$tenantId,$startDate,'active',$createdBy)`, { propertyId: space.property_id, spaceId, leaseId, tenantId, startDate: text(formData, "startDate") || today(), createdBy: actor.id });
...
export async function releaseSpaceAllocationAction(formData) {
  ...
  const allocation = get(`SELECT sa.*,rs.code,rs.unit_id FROM space_allocations sa JOIN rentable_spaces rs ON rs.id=sa.space_id WHERE sa.id=$allocationId`, { allocationId });
  if (!allocation || allocation.status !== "active") throw new Error("Active allocation not found");
  assertPermission(actor, "inventory.manage", allocation.property_id);
  assertPermission(actor, "agreements.manage", allocation.property_id);
  transaction(() => {
    const changed = run("UPDATE space_allocations SET status='ended',end_date=$endDate,updated_at=CURRENT_TIMESTAMP WHERE id=$allocationId AND status='active'", { allocationId, endDate: text(formData, "endDate") || today() });
    if (Number(changed.changes) !== 1) throw new Error("Allocation was already released");
