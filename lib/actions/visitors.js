...
function visitorWindow(formData) {
  const expectedAt = localDateTime(formData, "expectedAt", true);
  const expectedCheckout = localDateTime(formData, "expectedCheckout");
  if (expectedCheckout && expectedCheckout <= expectedAt) throw new Error("Expected checkout must be after expected arrival");
  return { expectedAt, expectedCheckout };
}
