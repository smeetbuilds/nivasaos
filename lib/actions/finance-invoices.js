...
import { moneyInput, normalizedMoney, toMinorUnits } from "@/lib/money";
...
export async function createInvoiceAction(formData) {
  ...
  if (leaseId && tenantId && !get("SELECT 1 FROM lease_tenants WHERE lease_id=$leaseId AND tenant_id=$tenantId", { leaseId, tenantId })) throw new Error("Tenant is not assigned to the selected lease");
  const invoiceAmount = moneyInput(formData, "amount", { label: "Invoice amount", minMinor: 1 });
  ...
  transaction(() => {
    const result = run(
      ...
      { propertyId, leaseId, tenantId, number: numberValue, description, issueDate, dueDate, amount: invoiceAmount.value }
    );
    recordAudit({ actor: user, action: "create", entityType: "invoice", entityId: Number(result.lastInsertRowid), propertyId, summary: `Created invoice ${numberValue}`, metadata: { amount: invoiceAmount.value, amountMinor: invoiceAmount.minor, dueDate, leaseId, tenantId } });
  });
...
export async function createRentRunAction(formData) {
  ...
  const result = transaction(() => {
    ...
    for (const lease of leases) {
      const rentAmount = normalizedMoney(lease.monthly_rent, "Monthly rent");
      if (toMinorUnits(rentAmount, "Monthly rent") <= 0) { skipped += 1; continue; }
      const inserted = run(
        ...
        { propertyId: lease.property_id, leaseId: lease.id, tenantId: lease.tenant_id || null, number: uid("INV"), description: `Rent · ${label}`, issueDate, dueDate: rentDueDate(period, lease.billing_day), amount: rentAmount, period }
      );
      if (Number(inserted.changes || 0) === 1) {
        created += 1;
        recordAudit({ actor: user, action: "generate", entityType: "invoice", entityId: Number(inserted.lastInsertRowid), propertyId: lease.property_id, summary: `Generated ${label} rent invoice for ${lease.reference}`, metadata: { amount: rentAmount, amountMinor: toMinorUnits(rentAmount), period, leaseId: lease.id } });
      } else skipped += 1;
    }
