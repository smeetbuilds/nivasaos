...
export async function loginAction(formData) {
  ...
  if (!valid) {
    if (user && user.status === "active" && !locked) {
      ...
      const lockUntil = nextFailures >= LOGIN_FAILURE_LIMIT ? new Date(Date.now() + LOGIN_LOCK_MINUTES * 60000).toISOString() : null;
      run(`UPDATE users SET failed_attempts=$failedAttempts,locked_until=$lockedUntil,updated_at=CURRENT_TIMESTAMP WHERE id=$userId`, { failedAttempts: nextFailures, lockedUntil: lockUntil, userId: user.id });
    }
