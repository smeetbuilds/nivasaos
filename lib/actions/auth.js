...
import { createSession, destroySession, hashPassword, isInstalled, verifyPasswordOrDummy } from "@/lib/auth";
import { get, run, transaction } from "@/lib/db";
import { recordAudit } from "@/lib/audit";
import { assertInstallationToken } from "@/lib/runtime-config";
import { passwordInput, text } from "@/lib/actions/shared";
...
export async function installAction(formData) {
  ...
  const country = text(formData, "country") || "Not specified";
  const currency = text(formData, "currency", true);
  const timezone = assertTimeZone(text(formData, "timezone", true));
  ...
  if (name.length > 160 || company.length > 160) throw new Error("Name and company must be 160 characters or fewer");
  ...
  if (country.length > 100) throw new Error("Country is too long");
  ...
  try {
    ownerId = transaction(() => {
      ...
      if (formData.get("demo") === "on") {
        ...
        const property = run(
          `INSERT INTO properties (name,type,module_id,address,city,country,currency)
           VALUES ($name,$type,$moduleId,'1 Demo Avenue','Demo City',$country,$currency)`,
          { name: `${module.shortLabel} Demo`, type: module.propertyType, moduleId: module.id, country, currency }
        );
      ...
      recordAudit({ actor, action: "create", entityType: "installation", summary: `Installed NivasaOS for ${company}`, metadata: { country, currency, timezone, moduleIds, primaryModule, configuredModules: Object.keys(defaults), demo: formData.get("demo") === "on" } });
      return id;
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (message.includes("installation_state") || message.includes("Installation is already complete")) throw new Error("Installation is already complete or another installer is running");
    throw error;
  }
...
export async function loginAction(formData) {
  ...
  const email = text(formData, "email", true).toLowerCase().slice(0, 254);
  const password = passwordInput(formData, "password");
  const throttle = await loginThrottleContext("staff", email);
  const retryAfter = throttleRetryAfter(throttle);
  const user = get("SELECT * FROM users WHERE email=$email", { email });
  const legacyLockUntil = user?.locked_until ? new Date(user.locked_until).getTime() : 0;
  const legacyLocked = Number.isFinite(legacyLockUntil) && legacyLockUntil > Date.now();
  const passwordValid = retryAfter === 0 && !legacyLocked ? verifyPasswordOrDummy(password, user?.password_hash) : false;
  const valid = retryAfter === 0 && !legacyLocked && Boolean(user && user.status === "active" && passwordValid);
  if (!valid) {
    if (retryAfter === 0 && !legacyLocked) recordAuthFailure(throttle);
    if (user?.status === "active" && !legacyLocked) {
      run("UPDATE users SET failed_attempts=failed_attempts+1,locked_until=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=$userId", { userId: user.id });
    }
  ...
  run("UPDATE users SET failed_attempts=0,locked_until=NULL,last_login_at=$now,updated_at=CURRENT_TIMESTAMP WHERE id=$userId", { now: nowIso, userId: user.id });
