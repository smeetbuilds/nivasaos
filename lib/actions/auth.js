...
function rawPassword(formData, key) {
...
export async function loginAction(formData) {
  ...
  const passwordValid = verifyPasswordOrDummy(password, user?.password_hash);
  const valid = retryAfter === 0 && Boolean(user && user.status === "active" && passwordValid);
  if (!valid) {
    if (retryAfter === 0) recordAuthFailure(throttle);
    ...
    const message = retryAfter > 0 ? "Sign in is temporarily unavailable. Try again later" : "Invalid email or password";
