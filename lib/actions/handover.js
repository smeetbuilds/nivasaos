...
export async function shareInspectionAction(formData) {
  ...
  const inspectionId = integer(formData, "inspectionId");
  const inspection = accessibleInspection(actor, inspectionId);
  if (inspection.status === "completed") throw new Error("Completed inspections cannot be shared again");
...
export async function acknowledgeInspectionAction(formData) {
  ...
  const inspection = get(
    `SELECT pi.* FROM property_inspections pi
     ...
     WHERE pi.id=$inspectionId AND lt.tenant_id=$tenantId AND pi.status IN ('shared','acknowledged')`,
  ...
  transaction(() => {
    run(
      `INSERT INTO inspection_acknowledgements (inspection_id,tenant_id,statement,tenant_note,acknowledged_at)
       ...
       ON CONFLICT(inspection_id,tenant_id) DO UPDATE SET statement=excluded.statement,tenant_note=excluded.tenant_note,acknowledged_at=CURRENT_TIMESTAMP`,
    ...
    run(
      "UPDATE property_inspections SET status=$status,updated_at=CURRENT_TIMESTAMP WHERE id=$inspectionId AND status!='completed'",
