...
export async function createLeaseAction(formData) {
  ...
  const validTenants = all(
    `SELECT id FROM tenants WHERE property_id=? AND status='active' AND id IN (${tenantIds.map(() => "?").join(",")})`,
    [propertyId, ...tenantIds]
  );
  ...
  transaction(() => {
    ...
    if (status === "active" && spaceInventory && requestedSpaceIds.length) {
      const selectedRows = all(
        `SELECT id,code,monthly_rate,deposit,gender_policy FROM rentable_spaces
         WHERE property_id=? AND unit_id=? AND status='available'
         AND id IN (${requestedSpaceIds.map(() => "?").join(",")})`,
        [propertyId, unitId, ...requestedSpaceIds]
      );
      ...
      allocatedSpaces = all(
        `SELECT id,code,monthly_rate,deposit,gender_policy FROM rentable_spaces
         WHERE property_id=$propertyId AND unit_id=$unitId AND status='available' AND gender_policy='any'
         ORDER BY id LIMIT $limit`,
        { propertyId, unitId, limit: tenantIds.length }
      );
    ...
    const pricing = resolveAgreementPricing({
      ...
      unitRate: unit.monthly_rate,
      unitDeposit: unit.deposit,
      requestedRent: text(formData, "monthlyRent"),
      requestedDeposit: text(formData, "deposit")
    });
    ...
    tenantIds.forEach((tenantId, index) => {
      ...
      if (status === "active" && spaceInventory) {
        ...
        run(
          ...
          { propertyId, spaceId, leaseId, tenantId, startDate, createdBy: actor.id }
        );
    ...
    recordAudit({
      ...
      entityId: leaseId,
      propertyId,
      summary: `Created lease ${reference}`,
      metadata: {
        unitId,
        tenantCount: tenantIds.length,
        status,
        moduleId: unit.module_id,
        allocatedSpaceIds: allocatedSpaces.map((space) => Number(space.id)),
        explicitSpaceSelection: requestedSpaceIds.length > 0,
