...
function refreshLeaseViews() {
  ["/leases", "/handover", "/units", "/tenants", "/dashboard", "/audit", "/portal", "/portal/lease"].forEach(revalidatePath);
}
...
export async function createLeaseAction(formData) {
  ...
  transaction(() => {
    ...
    recordAudit({ actor, action: "create", entityType: "lease", entityId: leaseId, propertyId, summary: `Created lease ${reference}`, metadata: { unitId, tenantCount: tenantIds.length, status } });
  });
  refreshLeaseViews();
...
export async function endLeaseAction(formData) {
  ...
  transaction(() => {
    ...
    recordAudit({ actor, action: "end", entityType: "lease", entityId: leaseId, propertyId: lease.property_id, summary: `Completed move-out for ${lease.reference}`, metadata: { unitId: lease.unit_id, depositHeldAtMoveOut: depositHeld } });
  });
  refreshLeaseViews();
