...
export async function endLeaseAction(formData) {
  ...
  transaction(() => {
    ...
    recordAudit({ actor, action: "end", entityType: "lease", entityId: leaseId, propertyId: lease.property_id, summary: `Completed move-out for ${lease.reference}`, metadata: { unitId: lease.unit_id } });
  });
  revalidatePath("/leases");
  revalidatePath("/units");
