...
function serviceAccess(actor, serviceId) {
  ...
  if (!service || !canAccessProperty(actor, service.property_id)) throw new Error("Service access denied");
...
export async function createServiceAction(formData) {
  ...
  if (!property || !canAccessProperty(actor, propertyId)) throw new Error("Property access denied");
  ...
  const values = {
    ...
    billingFrequency: choice(formData, "billingFrequency", FREQUENCIES, "monthly"),
    amount: Math.max(0, number(formData, "amount", 0)),
  ...
  transaction(() => {
    ...
    recordAudit({ actor, action: "create", entityType: "service_catalog", entityId: Number(inserted.lastInsertRowid), propertyId, summary: `Created service ${values.name}`, metadata: { frequency: values.billingFrequency, amount: values.amount } });
  });
  refreshServiceViews();
...
export async function updateServiceAction(formData) {
  ...
  const after = {
    name: limited(formData, "name", 160, true),
    category: limited(formData, "category", 80) || "other",
    billing_frequency: choice(formData, "billingFrequency", FREQUENCIES, before.billing_frequency),
    amount: Math.max(0, number(formData, "amount", before.amount)),
    description: limited(formData, "description", 1800),
  ...
  transaction(() => {
    ...
    recordAudit({ actor, action: "update", entityType: "service_catalog", entityId: serviceId, propertyId: before.property_id, summary: `Updated service ${after.name}`, metadata: { fields } });
  });
  refreshServiceViews();
...
export async function subscribeServiceAction(formData) {
  ...
  const customAmount = customAmountText === "" ? null : Math.max(0, number(formData, "customAmount", service.amount));
  transaction(() => {
    const inserted = run(
    ...
    recordAudit({ actor, action: "create", entityType: "lease_service", entityId: Number(inserted.lastInsertRowid), propertyId: service.property_id, summary: `Assigned service ${service.name}`, metadata: { leaseId, tenantId, customAmount } });
  });
  refreshServiceViews();
...
export async function endServiceSubscriptionAction(formData) {
  ...
  if (!subscription || subscription.status !== "active" || !canAccessProperty(actor, subscription.property_id)) throw new Error("Active service subscription access denied");
  transaction(() => {
    run("UPDATE lease_services SET status='ended',end_date=$endDate,updated_at=CURRENT_TIMESTAMP WHERE id=$subscriptionId AND status='active'", { subscriptionId, endDate: validDate(text(formData, "endDate") || today(), "Service end date") });
...
export async function billServiceSubscriptionAction(formData) {
  ...
  if (!subscription || subscription.status !== "active" || subscription.lease_status !== "active" || !canAccessProperty(actor, subscription.property_id)) throw new Error("Billable subscription access denied");
  if (subscription.billing_frequency === "included") throw new Error("Included services do not create separate invoices");
  const requestedPeriod = text(formData, "period") || today().slice(0, 7);
  const period = subscription.billing_frequency === "one_time" ? "one-time" : requestedPeriod;
  if (period !== "one-time" && !/^\d{4}-\d{2}$/.test(period)) throw new Error("Billing period must use YYYY-MM");
  const amount = Number(subscription.custom_amount ?? subscription.service_amount);
  ...
  transaction(() => {
    ...
    const inserted = run(
       ...
       VALUES ($propertyId,$leaseId,$tenantId,$number,$description,$issueDate,$dueDate,$amount,'manual','issued')`,
      {
        ...
        leaseId: subscription.lease_id,
        tenantId,
        number: invoiceNumber,
        description: `${subscription.service_name} · ${period}`,
        issueDate: today(),
        dueDate,
        amount
      }
    ...
    recordAudit({ actor, action: "create", entityType: "service_invoice", entityId: invoiceId, propertyId: subscription.property_id, summary: `Billed ${subscription.service_name} for ${period}`, metadata: { subscriptionId, amount, invoiceNumber } });
  });
  refreshServiceViews();
