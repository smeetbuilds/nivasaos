...
export async function updateSettingsAction(formData) {
  ...
  const values = Object.fromEntries(GENERAL_SETTING_KEYS.map((key) => [key, text(formData, key)]));
  ...
  values.brand_name = text(formData, "brand_name").slice(0, 80) || "NivasaOS";
  values.brand_tagline = text(formData, "brand_tagline").slice(0, 120) || "Property operations";
  values.white_label_enabled = formData.get("white_label_enabled") === "on" ? "1" : "0";
