...
export async function updateSettingsAction(formData) {
  ...
  transaction(() => {
    allowed.forEach((key) => run(
      "INSERT INTO settings (key,value,updated_at) VALUES ($key,$value,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP",
