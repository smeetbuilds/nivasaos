...
import { assertTimeZone, invalidateWorkspaceLocalizationCache } from "@/lib/workspace-localization";
...
export async function updateSettingsAction(formData) {
  ...
  try {
    ...
    fields = keys.filter((key) => String(before[key] || "") !== String(values[key] || ""));
    if (!fields.length) {
  ...
  if (fields.includes("timezone")) invalidateWorkspaceLocalizationCache();
