...
import { assertGlobalPermission, assertPermission } from "@/lib/permission-core";
...
function workspaceDefaults() {
  const settings = Object.fromEntries(all("SELECT key,value FROM settings WHERE key IN ('default_country','default_currency')").map((row) => [row.key, row.value]));
  return {
    country: String(settings.default_country || "Not specified"),
    currency: CURRENCIES.includes(settings.default_currency) ? settings.default_currency : "USD"
  };
}
...
export async function createPropertyAction(formData) {
  ...
  const values = {
    ...
    address: text(formData, "address", true), city: text(formData, "city"), country: text(formData, "country") || defaults.country,
    currency: choice(formData, "currency", CURRENCIES, defaults.currency)
  };
  const propertyId = transaction(() => {
    ...
    if (actor.role !== "owner") run("INSERT OR IGNORE INTO user_properties (user_id,property_id) VALUES ($userId,$propertyId)", { userId: actor.id, propertyId: id });
    const inherited = get("SELECT settings_json FROM workspace_modules WHERE module_id=$moduleId", { moduleId: module.id })?.settings_json || "{}";
    run(`INSERT INTO property_operating_configs (property_id,module_id,settings_json,configured_by) VALUES ($propertyId,$moduleId,$settingsJson,$actorId)`, { propertyId: id, moduleId: module.id, settingsJson: inherited, actorId: actor.id });
...
export async function updatePropertyAction(formData) {
  ...
  if (before.module_id !== module.id) {
    const operationalRows = Number(scalar(`SELECT (SELECT COUNT(*) FROM units WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM tenants WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM leases WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM invoices WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM payments WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM deposit_transactions WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM maintenance_tickets WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM billing_policies WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM property_module_settings WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM service_catalog WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM visitor_entries WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM property_inspections WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM lease_documents WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM lease_key_transactions WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM notification_log WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM property_operating_configs WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM resident_vertical_profiles WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM module_requests WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM hostel_reservations WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM housekeeping_tasks WHERE property_id=$propertyId)+(SELECT COUNT(*) FROM bulk_jobs WHERE property_id=$propertyId)`, { propertyId }) || 0);
  ...
  const after = {
    ...
    city: text(formData, "city"), country: text(formData, "country") || before.country || "Not specified", currency: choice(formData, "currency", CURRENCIES, before.currency),
...
export async function updateUnitAction(formData) {
  ...
  assertPermission(actor, "inventory.manage", before.property_id);
...
export async function createTenantAction(formData) {
  const actor = await requireUser();
  const propertyId = await assertProperty(formData, actor);
  assertPermission(actor, "people.manage", propertyId);
...
export async function updateTenantAction(formData) {
  ...
  assertPermission(actor, "people.manage", before.property_id);
