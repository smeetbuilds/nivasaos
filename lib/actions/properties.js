...
function refreshPropertyViews() {
  ["/properties", "/units", "/tenants", "/tenant-portal", "/leases", "/spaces", "/services", "/visitors", "/commercial", "/invoices", "/payments", "/maintenance", "/reports", "/dashboard", "/modules", "/audit", "/portal", "/portal/profile", "/portal/services", "/portal/visitors"].forEach(revalidatePath);
}
  ...
  const propertyId = transaction(() => {
    const result = run(
      `INSERT INTO properties (name,type,module_id,address,city,country,currency)
       VALUES ($name,$type,$moduleId,$address,$city,$country,$currency)`,
      values
    );
    ...
    recordAudit({ actor, action: "create", entityType: "property", entityId: id, propertyId: id, summary: `Created ${module.shortLabel} property ${values.name}`, metadata: { moduleId: module.id, seeded } });
    return id;
  });
...
export async function updatePropertyAction(formData) {
  ...
  if (before.module_id !== module.id) {
    const operationalRows = Number(scalar(
      `SELECT
       ...
       (SELECT COUNT(*) FROM tenants WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM leases WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM invoices WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM payments WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM deposit_transactions WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM maintenance_tickets WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM billing_policies WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM property_module_settings WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM service_catalog WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM visitor_entries WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM property_inspections WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM lease_documents WHERE property_id=$propertyId)+
       (SELECT COUNT(*) FROM lease_key_transactions WHERE property_id=$propertyId)+
  ...
  transaction(() => {
    run(
      `UPDATE properties SET name=$name,type=$type,module_id=$module_id,address=$address,city=$city,country=$country,currency=$currency,status=$status,updated_at=CURRENT_TIMESTAMP
       WHERE id=$propertyId`,
      { ...after, propertyId }
    );
...
export async function createUnitAction(formData) {
  ...
  const values = {
    propertyId,
    name: text(formData, "name", true),
    unitType: text(formData, "unitType") || moduleById(property.module_id).terminology.unit,
