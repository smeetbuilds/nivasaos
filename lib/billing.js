...
export function eligibleLateFeeInvoices(user, propertyId = null) {
  ...
  return all(
    `SELECT i.id,i.property_id,i.lease_id,i.tenant_id,i.number,i.due_date,i.amount,i.amount_paid,
      (i.amount-i.amount_paid) balance,p.name property_name,p.currency,
      bp.grace_days,date(i.due_date, '+' || bp.grace_days || ' days') grace_ends,bp.late_fee_type,bp.late_fee_value,bp.late_fee_cap,
