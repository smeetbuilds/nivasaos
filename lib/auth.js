...
import { currentPermissionScope } from "@/lib/permission-context";
import { permissionRequirementsScopeSql } from "@/lib/permission-core";
...
const DUMMY_PASSWORD_HASH = hashPassword("nivasaos-nonexistent-account-password-check");
...
export function propertyScopeSql(user, alias = "p") {
  const permissionScope = currentPermissionScope();
  if (permissionScope) return permissionRequirementsScopeSql(user, permissionScope, alias);
