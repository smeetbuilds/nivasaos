...
export function applyMigrations(database) {
  ...
  if (!hasColumn(database, "properties", "module_id")) {
  ...
  database.exec(`
    ...
    UPDATE properties SET module_id=CASE
      ...
      WHEN type='rental' THEN 'residential'
      WHEN type='apartment' THEN 'residential'
      ELSE 'residential'
    END WHERE module_id IS NULL OR module_id='';
