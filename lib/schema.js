...
CREATE TABLE IF NOT EXISTS invoices (
  ...
  source_invoice_id INTEGER REFERENCES invoices(id) ON DELETE RESTRICT,
...
export function applyMigrations(database) {
  ...
  if (!hasColumn(database, "invoices", "source_invoice_id")) {
    database.exec("ALTER TABLE invoices ADD COLUMN source_invoice_id INTEGER REFERENCES invoices(id) ON DELETE RESTRICT");
  }
  ...
  database.exec(`
    UPDATE invoices SET charge_type='rent'
    WHERE rent_period IS NOT NULL AND charge_type='manual';
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_invoices_active_late_fee
      ON invoices(source_invoice_id)
      WHERE source_invoice_id IS NOT NULL AND charge_type='late_fee' AND status != 'void';
    CREATE INDEX IF NOT EXISTS idx_invoices_charge_type ON invoices(property_id, charge_type, status);
    CREATE INDEX IF NOT EXISTS idx_invoices_source ON invoices(source_invoice_id);
    CREATE INDEX IF NOT EXISTS idx_audit_created ON audit_log(created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_audit_property ON audit_log(property_id, created_at DESC);
    CREATE TRIGGER IF NOT EXISTS trg_invoices_classify_rent
      AFTER INSERT ON invoices
      WHEN NEW.rent_period IS NOT NULL AND NEW.charge_type='manual'
      BEGIN
        UPDATE invoices SET charge_type='rent' WHERE id=NEW.id;
      END;
  `);
}
