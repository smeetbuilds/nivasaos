...
export const schema = `${coreSchema}${financeSchema}${operationsSchema}${handoverSchema}${moduleSchema}`;
...
export function applyMigrations(database) {
  ...
  if (addedPropertyModule) {
    ...
    database.exec(`
      UPDATE properties SET module_id=CASE
        WHEN type='boarding_house' THEN 'pg_coliving'
        ELSE 'residential'
      END;
    `);
  } else {
  ...
  database.exec(`
    ...
    CREATE TRIGGER IF NOT EXISTS trg_properties_module_insert
    BEFORE INSERT ON properties
    WHEN NEW.module_id NOT IN ('residential','pg_coliving','hostel','student_housing','staff_housing','commercial')
    BEGIN
      SELECT RAISE(ABORT,'invalid property module');
    END;

    CREATE TRIGGER IF NOT EXISTS trg_properties_module_update
    ...
    CREATE TRIGGER IF NOT EXISTS trg_properties_module_activity_lock
    BEFORE UPDATE OF module_id ON properties
    WHEN OLD.module_id != NEW.module_id AND (
      ...
      EXISTS (SELECT 1 FROM notification_log WHERE property_id=OLD.id)
    )
    BEGIN
      SELECT RAISE(ABORT,'property module is locked after configuration or activity');
    END;
