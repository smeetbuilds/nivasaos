...
export function recordAuthFailure(context, now = new Date()) {
  ...
  transaction(() => {
    for (const item of context) {
      ...
      const expiredLock = Boolean(row?.locked_until) && (!Number.isFinite(currentLock) || currentLock <= nowMs);
      const resetWindow = expiredLock || !Number.isFinite(windowStarted) || windowStarted <= nowMs - item.windowMs;
