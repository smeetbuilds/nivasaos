...
async function requestAddress() {
  ...
  const forwarded = String(store.get("x-forwarded-for") || "").split(",")[0].trim();
  const address = forwarded || String(store.get("x-real-ip") || "").trim() || "unknown";
...
export async function loginThrottleContext(realm, identifier) {
  ...
  const network = await requestAddress();
  return [
    { keyHash: digest(`${normalizedRealm}:account:${normalizedAccount}`), realm: normalizedRealm, dimension: "account", ...POLICIES.account },
    { keyHash: digest(`${normalizedRealm}:network:${network}`), realm: normalizedRealm, dimension: "network", ...POLICIES.network }
  ];
}
