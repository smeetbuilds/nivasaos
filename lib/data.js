...
export function dashboardData(user) {
  ...
  return { totalProperties, units, moneyByCurrency, overdueByCurrency, maintenance, recentInvoices, recentTickets };
}
...
export function reportData(user, propertyId) {
  ...
  const params = { ...scope.params };
  if (propertyId) { filters.push("p.id = $propertyId"); params.propertyId = Number(propertyId); }
