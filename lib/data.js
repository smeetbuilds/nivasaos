...
export function dashboardData(user) {
  ...
  const paymentsByCurrency = all(`SELECT p.currency,COALESCE(SUM(x.amount),0) collected FROM payments x ${join(paymentScope)} WHERE x.paid_at>=date('now','start of month') GROUP BY p.currency ORDER BY p.currency`, paymentScope.params);
  const overdueByCurrency = all(`SELECT p.currency,COUNT(*) count,COALESCE(SUM(x.amount-x.amount_paid),0) balance FROM invoices x ${join(billingScope)} WHERE x.status NOT IN ('paid','void') AND x.due_date<date('now') GROUP BY p.currency ORDER BY p.currency`, billingScope.params);
  ...
  const leaseExpiries = all(
    `SELECT x.id,x.reference,x.end_date,p.name property_name,u.name unit_name,
     ...
     ORDER BY x.end_date
     LIMIT 6`,
    agreementScope.params
  );
  const lateFees = lateFeeSummary(user);
