...
export default function InstallWizard({ installationProtection = { required: false, configured: true } }) {
  ...
  function canContinue() {
    ...
    if (step === 1) return workspace.company.trim().length > 1 && workspace.country.trim().length > 1 && workspace.currency && workspace.timezone.trim().length > 1;
