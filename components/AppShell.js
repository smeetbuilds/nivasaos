...
function buildNavigation(capabilities, modules) {
  ...
  return [
    ...
    { label: "Portfolio", items: [
      ...
      ...(has("spaceInventory") ? [["/spaces","spaces","Beds & spaces","inventory.manage&agreements.manage"]] : []),
...
function routeIsActive(pathname, href) { return pathname === href || pathname.startsWith(`${href}/`); }
function Brand({ branding, compact = false }) { return <Link href="/dashboard" className={`brand${compact ? " brand-compact" : ""}`} aria-label={`${branding.name} dashboard`}><BrandLogo branding={branding} variant="dark" className={compact ? "brand-logo-compact" : ""}/></Link>; }
function Navigation({ sections, pathname, onNavigate, mobile = false }) { return <nav className={mobile ? "drawer-nav" : "side-nav"} aria-label={mobile ? "Mobile navigation" : "Primary navigation"}>{sections.map((section) => <div className="nav-section" key={section.label}><span className="nav-section-label">{section.label}</span><div className="nav-section-items">{section.items.map(([href, icon, label]) => { const active = routeIsActive(pathname, href); return <Link key={href} href={href} className={active ? "is-active" : ""} aria-current={active ? "page" : undefined} onClick={onNavigate}><span className="nav-icon"><Icon name={icon} size={18}/></span><span>{label}</span>{active && <span className="active-indicator" aria-hidden="true"/>}</Link>; })}</div></div>)}</nav>; }
function UserCard({ user, mobile = false }) { return <div className={`user-card${mobile ? " user-card-mobile" : ""}`}><span className="avatar">{user.name.slice(0,1).toUpperCase()}</span><span className="user-copy"><strong>{user.name}</strong><small>{user.role}</small></span></div>; }
function ModuleStrip({ modules }) { return <div className="shell-module-strip" aria-label="Accessible operating modules">{modules.slice(0,4).map((module) => <span className={`module-${module.id}`} key={module.id} title={module.label}><Icon name={module.icon} size={14}/></span>)}{modules.length > 4 && <small>+{modules.length - 4}</small>}</div>; }

export default function AppShell({ user, company, branding, modules = [], capabilities = [], permissions = [], children }) {
  ...
  useEffect(() => {
    ...
    if (drawerOpen) { drawerWasOpenRef.current = true; drawerCloseRef.current?.focus(); }
    else if (drawerWasOpenRef.current) { drawerWasOpenRef.current = false; menuButtonRef.current?.focus(); }
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><header className="topbar"><div className="mobile-topbar-start"><BrandLogo branding={branding} variant="light" symbol className="mobile-brand-symbol"/><button ref={menuButtonRef} type="button" className="mobile-menu-button" onClick={() => setDrawerOpen(true)} aria-label="Open navigation" aria-expanded={drawerOpen} aria-controls="mobile-navigation-drawer"><Icon name="menu" size={22}/></button><div className="mobile-page-title"><span>{current?.[2] || "Workspace"}</span><small>{company}</small></div></div><div className="desktop-topbar-start"><span className="topbar-kicker">{company}</span><strong className="company-name">{current?.[2] || "Workspace"}</strong></div><div className="topbar-meta"><span className="status-pill module-count-pill"><Icon name="modules" size={14}/>{modules.length} module{modules.length === 1 ? "" : "s"}</span><span className="status-pill"><span className="live-dot"/>Self-hosted</span><span className="topbar-user"><span className="avatar avatar-small">{user.name.slice(0,1).toUpperCase()}</span><span><strong>{user.name}</strong><small>{user.role}</small></span></span></div></header><main className="content">{children}</main><footer className="footer">{branding.whiteLabel ? <><span>{branding.name}</span><span>•</span><span>Self-hosted operations</span></> : <>{branding.name} by <a href="https://aahavlabs.in" target="_blank" rel="noreferrer">Aahav Labs</a><span>•</span><a href="mailto:hi@aahavlabs.in">Support</a></>}</footer></div>
