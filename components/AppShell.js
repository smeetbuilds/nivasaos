...
function Brand({ compact = false }) { return <Link href="/dashboard" className={`brand${compact ? " brand-compact" : ""}`}><span className="brand-mark"><Icon name="building" size={compact ? 19 : 22}/></span><span><strong>NivasaOS</strong><small>Property operations</small></span></Link>; }
...
export default function AppShell({ user, company, modules = [], capabilities = [], permissions = [], children }) {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><Brand/><div className="sidebar-context"><small>Workspace</small><strong>{company}</strong><span>{modules.length} operating model{modules.length === 1 ? "" : "s"} enabled</span></div><ModuleStrip modules={modules}/><Navigation sections={sections} pathname={pathname}/><div className="sidebar-bottom"><UserCard user={user}/><form action={logoutAction}><button className="logout-button" aria-label="Sign out"><Icon name="logout" size={18}/></button></form></div></aside>
